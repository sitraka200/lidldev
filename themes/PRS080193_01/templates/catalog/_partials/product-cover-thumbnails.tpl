...
{if $product.cover}
    ...
    {* NEW CHANGE *}
    {* <div>{$product.images[0].id_image|var_dump}</div> *}
    {if is_string($link) }
      <img class="js-qv-product-cover" src="{$product.cover.bySize.large_default.url}" alt="{$product.cover.legend}" title="{$product.cover.legend}" style="width:100%;" itemprop="image">
    {else}
      <img class="js-qv-product-cover" src="{$link->getImageLink($product.link_rewrite, $product.images[0].id_image, NULL)}" alt="{$product.cover.legend}" title="{$product.cover.legend}" style="width:100%;" itemprop="image">
    {/if}
