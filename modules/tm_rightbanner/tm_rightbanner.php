...
include_once(_PS_MODULE_DIR_.'tm_rightbanner/RightBanner.php');

class Tm_RightBanner extends Module implements WidgetInterface
{
    protected $_html = '';
    ...
    public function __construct()
    {
        ...
        $this->displayName = $this->getTranslator()->trans('TM - Right Banner', array(), 'Modules.RightBanner');
        ...
        $this->templateFile = 'module:tm_rightbanner/views/templates/hook/tmrightbanner.tpl';
    }
    ...
    public function getSlides($active = null)
    {
        ...
        $slides = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
            SELECT hs.`id_tmrightbanner_slides` as id_slide, hss.`position`, hss.`active`, hssl.`title`,
            hssl.`url`, hssl.`image`
        ...
        foreach ($slides as &$slide) {
            $slide['image_url'] = $this->context->link->getMediaLink(_MODULE_DIR_.'tm_rightbanner/views/img/'.$slide['image']);
        }
    ...
    public function getAllImagesBySlidesId($id_slides, $active = null, $id_shop = null)
    {
        ...
        $results = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
            ...
            LEFT JOIN '._DB_PREFIX_.'tmrightbanner_slides hss ON (hs.id_tmrightbanner_slides = hss.id_tmrightbanner_slides)
            LEFT JOIN '._DB_PREFIX_.'tmrightbanner_slides_lang hssl ON (hss.id_tmrightbanner_slides = hssl.id_tmrightbanner_slides)
            WHERE hs.`id_tmrightbanner_slides` = '.(int)$id_slides.' AND hs.`id_shop` = '.(int)$id_shop.
            ($active ? ' AND hss.`active` = 1' : ' ')
        );
    ...
    public function renderList()
    {
        ...
        foreach ($slides as $key => $slide) {
            ...
            $associated_shop_ids = RightBanner::getAssociatedIdsShop((int)$slide['id_slide']);
