...
class AdminPsblogCategoriesController extends ModuleAdminController
{
    ...
    public function __construct()
    {
		...
		$this->toolbar_title = $this->module->getTranslator()->trans('Categories Management');
		// echo '<pre>';
		// print_r($this->module->getTranslator()->trans('Ps Blog Management'));die();
    }
    ...
    public function postProcess()
    {
        if (Tools::getValue('doupdatepos') && Tools::isSubmit('updatePosition')) {
            ...
            die($this->module->getTranslator()->trans('Update Positions Done'));
        }
        ...
        if (Tools::isSubmit('save'.$this->name) && Tools::isSubmit('active')) {
            ...
            if ($megamenu->validateFields(false) && $megamenu->validateFieldsLang(false)) {
                ...
                // validate module
                $this->_html .= '<div class="conf error alert alert-warning">'.$this->module->getTranslator()->trans('An error occurred while attempting to save.').'</div>';
            }
        }
    }
    ...
    public function renderList()
    {
        ...
        $this->fields_form[0]['form'] = array(
            'legend' => array(
                'title' => $this->module->getTranslator()->trans('Category Form.'),
            ),
            'input' => array(
                array(
                    'type' => 'hidden',
                    'label' => $this->module->getTranslator()->trans('Category ID'),
                    'name' => 'id_psblogcat',
                    'default' => 0,
                ),
                array(
                ...
                array(
                    ...
                    'lang' => true,
                    'default' => '',
                    'hint' => $this->module->getTranslator()->trans('Only letters and the minus (-) character are allowed')
                ),
                array(
                    'type' => 'select',
                    'label' => $this->module->getTranslator()->trans('Parent ID'),
                    'name' => 'id_parent',
                ...
                array(
                    'type' => 'switch',
                    'label' => $this->module->getTranslator()->trans('Is Active'),
                    'name' => 'active',
                    'values' => $soption,
                    'default' => '1',
                ),
                ...
                array(
                    'type' => 'text',
                    'label' => $this->module->getTranslator()->trans('Addion Css Class'),
                    'name' => 'menu_class',
                    'display_image' => true,
                    'default' => ''
                ),
                array(
                    'type' => 'text',
                    'label' => $this->module->getTranslator()->trans('Menu Icon Class'),
                    'name' => 'icon_class',
                    'display_image' => true,
                    'default' => '',
                    'desc' => $this->module->getTranslator()->trans('The module integrated with FontAwesome').'. '
                    .$this->module->getTranslator()->trans('Check list of icons and class name in here')
                    .' <a href="http://fontawesome.io/" target="_blank">http://fontawesome.io/</a> or your icon class'
                ),
                array(
                    'type' => 'file',
                    'label' => $this->module->getTranslator()->trans('Image'),
                    'name' => 'image',
                    'display_image' => true,
                    'default' => '',
                    'desc' => $this->module->getTranslator()->trans(''),
                    'thumb' => '',
                    'title' => $this->module->getTranslator()->trans('Icon Preview'),
                ),
                array(
                    'type' => 'textarea',
        ...
        $this->fields_form[1]['form'] = array(
            'legend' => array(
                'title' => $this->module->getTranslator()->trans('SEO META'),
            ),
            'input' => array(
                // custom template
                array(
        ...
        $helper->title = $this->module->getTranslator()->trans('Categories Management');
        ...
        $output = $html.'
	        ...
	        ';
        $output .= '<div class="col-md-4"><div class="panel panel-default"><h3 class="panel-title">'.$this->module->getTranslator()->trans('Tree Blog Categories Management').'</h3>'
                .'<div class="panel-content">'.$this->module->getTranslator()->trans('To sort orders or update parent-child, you drap and drop expected menu.')
                .'<hr><p><input type="button" value="'.$this->module->getTranslator()->trans('New Category').'" id="addcategory" data-loading-text="'.$this->module->getTranslator()->trans('Processing ...').'" class="btn btn-danger" name="addcategory"></p><hr>'.$tree.'</div></div></div>'
